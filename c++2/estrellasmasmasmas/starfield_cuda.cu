...
// - Motion blur temporal por acumulación exponencial
//
// Deps (Ubuntu):
...
// SIN límite máximo intencionadamente:
// antes había RADIUS_MAX_F = 16.0f;
...
// Control principal:
// valor equivalente aproximado al número de fotogramas que participan.
// Ejemplos:
// 1.0  = sin blur real
// 2.0  = blur muy ligero
// 6.0  = blur medio
// 12.0 = blur fuerte
static constexpr float MOTION_BLUR_ACCUM_FRAMES = 8.0f;
...
__global__ void project_stars_roll_only(
  ...
  if (sx < -2048.0f || sx > (W + 2048.0f) || sy < -2048.0f || sy > (H + 2048.0f)) {
    out[i] = d;
    return;
  }
...
__global__ void render_stars(float* fb, const DrawStar* d, int n) {
  ...
  // SIN CLAMP MÁXIMO:
  float core_r = fmaxf(CORE_R_MIN, r * CORE_RADIUS_FACTOR);
  ...
  // SIN CLAMP MÁXIMO:
  float glow_r = fmaxf(GLOW_R_MIN, r * GLOW_RADIUS_FACTOR);
...
// Acumulación temporal tipo EMA.
// alpha = 1 / N   => N ~ número equivalente de frames acumulados.
__global__ void temporal_accumulate(float* history, const float* current, int n, float alpha, int first_frame) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= n) return;
  ...
  if (first_frame) {
    ...
    float oldv = history[i];
    history[i] = oldv * (1.0f - alpha) + cur * alpha;
  }
}
...
int main() {
  ...
  const int64_t numPixels64 = int64_t(W) * int64_t(H);
  const int numPixels = (int)numPixels64;
  ...
  float* d_fb = nullptr;        // frame actual
  float* d_history = nullptr;   // acumulación temporal
  ...
  ck(cudaMalloc(&d_history, size_t(W) * size_t(H) * sizeof(float)), "malloc history");
  ...
  clear_fbuffer<<<Gpix, B>>>(d_history, numPixels, 0.0f);
  ...
  if (ENABLE_MOTION_BLUR) {
    std::cout << "Motion blur: ON"
              << "  equivalent accumulation frames ~= "
              << MOTION_BLUR_ACCUM_FRAMES << "\n";
  } else {
    std::cout << "Motion blur: OFF\n";
  }
  ...
  // alpha de acumulación
  const float blurFrames = fmaxf(1.0f, MOTION_BLUR_ACCUM_FRAMES);
  const float alpha = 1.0f / blurFrames;
  ...
  for (int64_t frame = 0; frame < total_frames && !g_stop.load(std::memory_order_relaxed); ++frame) {
    ...
    project_stars_roll_only<<<Gs, B>>>(
      ...
      RADIUS_SCALE, RADIUS_MIN_F
    );

    clear_fbuffer<<<Gpix, B>>>(d_fb, numPixels, 0.0f);
    render_background<<<Gbg, B>>>(d_fb, d_bg, NUM_BG_STARS);
    render_stars<<<Gs, B>>>(d_fb, d_draw, NUM_STARS);

    if (ENABLE_MOTION_BLUR) {
      temporal_accumulate<<<Gpix, B>>>(d_history, d_fb, numPixels, alpha, (frame == 0) ? 1 : 0);
      fb_to_u8<<<Gpix, B>>>(d_history, d_u8, numPixels);
    } else {
      fb_to_u8<<<Gpix, B>>>(d_fb, d_u8, numPixels);
    }
