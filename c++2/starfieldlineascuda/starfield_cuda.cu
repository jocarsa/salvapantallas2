...
// Output: single HEVC/H.265 MP4 encoded on the fly while rendering.
//
// Deps (Ubuntu):
...
static constexpr int   VIDEO_SECONDS = 10 * 60 * 60;
...
//   "libx265"    -> CPU H.265
static const char* HW_ENCODER = "hevc_nvenc";

// NVENC knobs
static const char* NVENC_PRESET = "p7"; // p7 = best quality/compression, slower
static constexpr int NVENC_CQ = 24;     // lower = better quality, larger file

// QSV knobs
static constexpr int QSV_BITRATE_Mbps = 12;

// libx265 knobs
static constexpr int X265_CRF = 24;
static const char* X265_PRESET = "slow";
...
// =========================
static constexpr float LINE_THRESH_3D = 10.0f;
...
static std::string make_out_path_10h() {
  std::ostringstream oss;
  oss << OUT_DIR << "/starfield_" << now_stamp() << "_hevc_10h.mp4";
  return oss.str();
}

// =========================
// FFmpeg pipe writer (NV12 -> HEVC/H.265)
// =========================
class FFmpegPipe {
public:
  ...
  void open() {
    ...
    if (std::string(HW_ENCODER) == "hevc_nvenc") {
      cmd
        << "ffmpeg -y "
        << "-loglevel warning "
        << "-f rawvideo "
        ...
        << "-pix_fmt yuv420p "
        << "\"" << out_path_ << "\"";
    } else if (std::string(HW_ENCODER) == "hevc_qsv") {
      cmd
        << "ffmpeg -y "
        << "-loglevel warning "
        << "-f rawvideo "
        ...
        << "-pix_fmt yuv420p "
        << "\"" << out_path_ << "\"";
    } else if (std::string(HW_ENCODER) == "libx265") {
      cmd
        << "ffmpeg -y "
        << "-loglevel warning "
        << "-f rawvideo "
      ...
      throw std::runtime_error("Unknown HW_ENCODER. Use hevc_nvenc, hevc_qsv or libx265.");
    }
...
__global__ void render_lines(float* fb, const LineSeg* lines, const int* counts, int nStars) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= nStars) return;
...
__global__ void render_stars(float* fb, const DrawStar* d, int n) {
  ...
  int half = (int)ceilf(3.25f * sigma);
...
int main() {
  ...
  std::cout << "Recording (10h HEVC): " << out_path << "\n";
  std::cout << "Encoder: " << HW_ENCODER << "\n";
  ...
  for (int64_t frame = 0; frame < total_frames && !g_stop.load(std::memory_order_relaxed); ++frame) {
    ...
    ck(cudaGetLastError(), "frame kernels");
    ...
    if (!writer.write_nv12(nv12.data(), nv12.size())) {
      std::cerr << "Stopping because ffmpeg pipe broke.\n";
      break;
    }
